...
(fig1b <- gravity_ci %>% 
    ...
    filter(measure == '(Cue - Control) / Total',
           total_worms > 1) %>% 
    ...
    geom_text(data = gravity_stat_layer %>% filter(measure == '(Cue - Control) / Total'),
              aes(label = sig)) +
...
(fig1c <- all_join %>%
    mutate(genotype = factor(genotype, levels = c('N2', '*tax-4(p678)*')),
           dummy = case_when(
             slice %in% c('Attractant', 'Repellant') ~ 1,
             slice == 'Top' ~ 2,
             slice == 'Middle' ~ 3,
    ...
    geom_label(data = split_line,
               ...
               x = 3.75, y = 0.82, size = 2.5, hjust = 0, fill = alpha('white', 0.25)) +
...
middle <- plot_grid(fig1b, fig1c, rel_widths = c(0.5, 1), 
                    align = 'h', axis = 'tb',
                    labels = c('B', 'C'), label_size = 12)

fig1 <- plot_grid(NULL, middle, fig1d, rel_heights = c(0.75, .8, 1),
